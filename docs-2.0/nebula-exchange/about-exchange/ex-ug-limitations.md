...
- Apache Spark。使用 Exchange 从不同数据源导出数据对 Spark 版本的要求如下：
  ...
  !!! note
        使用 Exchange 时，需根据 Spark 版本选择相应的 JAR 文件。例如，当 Spark 版本为 2.4 时，选择 nebula-exchange_spark_2.4-{{exchange.release}}.jar。Exchange 对 Spark 3.x 的 x 具体支持范围请参考代码仓库中的[版本兼容矩阵](https://github.com/vesoft-inc/nebula-exchange#version-compatibility-matrix)。
